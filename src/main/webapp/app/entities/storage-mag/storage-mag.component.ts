...
@Component({
    selector: 'jhi-storage-mag',
    templateUrl: './storage-mag.component.html'
})
export class StorageMagComponent implements OnInit, OnDestroy {
storages: StorageMag[];
    currentAccount: any;
        ...
        if (this.currentSearch) {
            this.storageService.search({
                query: this.currentSearch,
                    ...
                    (res: HttpResponse<StorageMag[]>) => this.storages = res.body,
                    (res: HttpErrorResponse) => this.onError(res.message)
                );
            return;
       }
        this.storageService.query().subscribe(
            (res: HttpResponse<StorageMag[]>) => {
    ...
    trackId(index: number, item: StorageMag) {
        return item.id;
    }
